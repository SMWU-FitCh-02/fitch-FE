...
export function difficultyTier(maxNote: number): DifficultyTier {
  if (maxNote <= 60) return { label: "쉬움", className: "bg-emerald-500/15 text-emerald-400", stars: 1 }
  if (maxNote <= 67) return { label: "보통", className: "bg-primary/15 text-primary", stars: 2 }
  return { label: "어려움", className: "bg-rose-500/15 text-rose-400", stars: 3 }
}
