...
## 11. 앨범 커버가 유명한 곡도 안 뜨던 문제 수정 (긴급 패치)

`country=KR` 제한 때문에 BTS Dynamite처럼 확실히 있어야 할 곡도 검색 결과가
0개로 나오는 경우가 있었어요.

- **app/api/artwork/route.ts** — KR 카탈로그로 먼저 검색하고, 결과가 없으면
  국가 제한 없이(기본 US 카탈로그) 한 번 더 검색하도록 변경.

## 12. 앨범 커버 rate limit 재발 방지 (긴급 패치 3)

곡이 많은 화면에서 여전히 일부 커버가 안 뜨는 문제가 재발 — 동시 요청 개수만
제한하는 걸로는 부족해서, 요청 사이에 시간 간격(250ms)을 두도록 강화.
또한 실패한 조회는 더 이상 캐싱하지 않도록 변경 (성공한 것만 캐싱) —
그래야 화면을 다시 열었을 때 재시도할 기회가 생김.

- **lib/artwork-cache.ts** — 동시 요청 3개→2개로 줄이고, 요청 사이 250ms
  간격 추가, 실패 결과는 캐싱 안 함.

## 13. 앨범 커버를 실시간 조회 대신 미리 캐싱 (구조 변경)

계속 재발하던 rate limit 문제의 근본 원인은, 화면을 열 때마다 곡 수만큼
iTunes에 실시간으로 요청을 날리는 구조 자체였음. 이걸 "한 번 미리 조회해서
파일로 저장, 앱은 그 파일을 먼저 참고" 하는 방식으로 바꿈.

- **fetch_artwork_cache.py** (별도 스크립트, 이 프로젝트 폴더 바깥에서 실행) —
  DB의 모든 곡에 대해 천천히(곡당 1.2초 간격) iTunes를 조회해서
  `artwork-cache.json`에 저장. 중간에 rate limit 걸리면 자동 대기 후 재시도,
  중단돼도 이어서 실행 가능.
- **app/api/artwork/route.ts** — `public/artwork-cache.json`을 먼저 확인하고,
  없는 곡만 실시간 조회로 대체(fallback). 대부분의 요청이 이제 iTunes를
  아예 안 거치므로 rate limit 위험이 크게 줄어듦.

**적용 방법**: `fetch_artwork_cache.py`를 실행해서 나온 `artwork-cache.json`
파일을 `public/artwork-cache.json` 위치에 놓아야 함 (다른 위치면 못 찾음).

## 14. 곡 상세페이지 저장(북마크) 버튼 실제로 작동하게 수정

곡 상세페이지 우측 상단 북마크 아이콘이 눌러도 아무 반응 없던 버그 수정.
기존에 로컬 목데이터용으로 있던 저장 로직(`toggleLibrary`)을 실제 백엔드
곡에도 재사용하도록 연결했어요.

- **app/(app)/songs/[id]/page.tsx** — 북마크 버튼에 `toggleLibrary` 연결,
  저장 여부에 따라 아이콘 채워짐(BookmarkCheck)/빈 아이콘(Bookmark) 표시.
- **app/(app)/library/page.tsx** — "보관함" 화면이 로컬 목데이터 곡뿐 아니라,
  실제 백엔드 곡 중 저장한 것도 같이 불러와서 보여주도록 수정.

**참고**: 곡 목록(추천곡 리스트)의 카드 자체에는 아직 저장 버튼이 안
붙어있어요 — 지금은 상세페이지에 들어가야만 저장할 수 있어요. 리스트
카드에도 저장 버튼 붙이고 싶으시면 말씀해주세요.
